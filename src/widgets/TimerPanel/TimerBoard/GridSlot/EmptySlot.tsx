...
const EmptySlot = ({data, onClick, className, animate}: IProps) => {
  ...
  return (
    <div className={cn('EmptySlot', {void: isVoid}, className)} onClick={(event) => handleClick(data, event)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} title={translate('add_tracked_item')}>
        {
          isVoid && 

...
const SettingsStage = (): JSX.Element => {
    ...
    const widthRatio = 191.1; // percent of height
    const heigthRatio = 52.3; // percent of width 

    const desiredWidth = 72.2;
    const desiredHeight = 74.4;

    let width = (dimensions.width / 100) * desiredWidth;
    let heigth = (dimensions.height / 100) * desiredHeight;
 
    const widthIsBigger = dimensions.width > dimensions.height;

    let mobilePortrait = false;

    if (widthIsBigger) {
        const calculatedWidth = (heigth / 100) * widthRatio;
        width = calculatedWidth < dimensions.width ? calculatedWidth : dimensions.width;
        mobilePortrait = false;
    } else {

        if (dimensions.width < 900) {
            mobilePortrait = true;
            const calculatedHeigth = (heigth / 100) * widthRatio;
            heigth = calculatedHeigth < dimensions.height ? calculatedHeigth : dimensions.height;
        } else {
            mobilePortrait = false;
            const calculatedHeigth = (width / 100) * heigthRatio;
            heigth = calculatedHeigth < dimensions.height ? calculatedHeigth : dimensions.height;
        }
    }
    ...
    return (
        <div className="Playground__inner">
            ...
            <div className="Playground__topSection">
                {
            ...
            <div className="Playground__middleSection">
                <div className={cn('Playground__slotContainer', {mobilePortrait: mobilePortrait})}style={{width: width, height: heigth}}>
                    <div className="Playground__slotBox"></div>
                    <div className="Playground__slotBox"></div>
                    <div className="Playground__slotBox"></div>
                    <div className="Playground__slotBox"></div>
                    <div className="Playground__slotBox"></div>

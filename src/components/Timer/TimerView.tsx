...
import useHotkeyStore from '../../store/global/hotkeysStore';
...
const TimerView = ({ability, appStatus, runApp, pauseApp, currentStage, removeTimer, withSound = true}: IProps): JSX.Element => {
    ...
    const getKey = useCallback((event: KeyboardEvent) => {
        ...
        assignKey(ability.position, keyIs)
    }, []);
    ...
    const getHotkey = useHotkeyStore((state) => state.getHotkey)
    const assignKey = useHotkeyStore((state) => state.assignHotkey)

    const _hotkey = getKeyFromCode(getHotkey(ability.position))
    ...
    const isTimerToShowSpellTools = !!outerContainer
    // <-- 02.02.26
